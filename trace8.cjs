...
for (let i = 9383; i <= 9795; i++) {
  ...
  for (let m of matches) {
      if (m.type === 'open') stack.push(i + 1);
      else if (stack.length > 0) {
         let popped = stack.pop();
         if (popped === 9471) console.log(`Grid cols (9471) closed at ${i + 1}`);
         if (popped === 9470) console.log(`Form Side (9470) closed at ${i + 1}`);
      }
  }
}
