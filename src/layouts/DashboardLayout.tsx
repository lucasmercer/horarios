...
export default function DashboardLayout() {
  ...
  const handleExportBackup = () => {
    ...
    // No indentation to keep file size as small as possible
    const blob = new Blob([JSON.stringify(exportObject)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    const dateStr = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
    link.download = `backup_completo_${dateStr}.json`;
  ...
  const handleImportBackup = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Basic validation
        const hasSomeData = data.localStorageData || data.teachers || data.subjects || data.turmas || data.schedules;
  ...
  const confirmImport = () => {
    ...
    if (data.localStorageData) {
      ...
      const sn = localStorage.getItem('cecm_school_name');
  ...
  const [hasSalas, setHasSalas] = useState(true);
  ...
  useEffect(() => {
    ...
    const checkData = () => {
      try {
        ...
        const regularTurmas = t.filter((item: any) => !item.isRoom);
        const specialRooms = t.filter((item: any) => item.isRoom);

        setHasTurmas(regularTurmas.length > 0);
        setHasDisciplinas(d.length > 0);
        setHasProfessores(p.length > 0);
        setHasSalas(specialRooms.length > 0);
      } catch (e) {}
    };
    checkData();
  ...
  const navItems = [
    ...
    { name: 'Salas Especiais', path: '/salas', icon: Building2, locked: !hasTurmas || !hasDisciplinas || !hasProfessores },
    { name: 'Horários', path: '/horarios', icon: CalendarDays, locked: !hasTurmas || !hasDisciplinas || !hasProfessores || !hasSalas },
    { name: 'Substituições', path: '/substituicoes', icon: CalendarClock, locked: !hasTurmas || !hasDisciplinas || !hasProfessores || !hasSalas },
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Importar arquivo .json</p>
                  </div>
                </div>
                <button 
              ...
              <div className="p-8 pt-4 space-y-5">
                <div className="text-xs text-slate-500 font-medium leading-relaxed">
                  Para restaurar os seus dados, clique no botão abaixo e selecione o <span className="font-bold text-slate-700">arquivo de backup (.json)</span> que você salvou anteriormente.
                </div>
              ...
              <div className="flex items-center justify-between p-6 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Exportar arquivo .json Completo</p>
                  </div>
                </div>
                <button 
              ...
              <div className="flex flex-1 overflow-hidden min-h-0 bg-slate-50">
                ...
                <div className="w-1/4 max-w-[240px] border-r border-slate-200 bg-white p-4 space-y-1.5 overflow-y-auto shrink-0 custom-scrollbar">
                  ...
                  {[
                    ...
                    { id: 'salas', label: '8. Instalações e Salas', icon: Building2 }
                  ].map(tab => {
                ...
                <div className="flex-1 p-8 overflow-y-auto bg-white custom-scrollbar">
                  ...
                  {manualActiveTab === 'certificados' && (
                    <div className="space-y-4 animate-in fade-in duration-200">
                      <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2 border-b-2 border-slate-100 pb-2">
                        <Award className="w-4 h-4 text-amber-500 px-0.5" />
                        7. Gerador de Certificados
                      </h4>
                      <div className="space-y-3 text-xs text-slate-600 font-sans leading-relaxed">
                        <p>
                          Módulo inteligente para personalização e emissão em lote de certificados oficiais em PDF (como Menção Honrosa, Aluno Destaque e Destaque Cívico-Militar). Traz modelos formatados, preenchimento automático das datas acadêmicas vigentes e impressão limpa de vias individuais ou coletivas.
                        </p>
                      </div>
                    </div>
                  )}

                  {manualActiveTab === 'salas' && (
                    <div className="space-y-4 animate-in fade-in duration-200">
                      <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2 border-b-2 border-slate-100 pb-2">
                        <Building2 className="w-4 h-4 text-emerald-600 px-0.5" />
                        8. Controle de Infraestrutura Física (Salas e Laboratórios)
                      </h4>
                      <div className="space-y-3 text-xs text-slate-600 font-sans leading-relaxed">
                        <p>
                          Painel central unificado para controle de espaços físicos e reservas extraordinárias na instituição:
                        </p>
                        <ul className="list-disc pl-5 space-y-1.5 font-sans">
                          <li><strong>Visualizador Semanal:</strong> Escolha uma sala especial para inspecionar todas as aulas do turno corrente, identificando onde há turmas alocadas curricularmente ou reservas livres.</li>
                          <li><strong>Panorama Diário:</strong> Intercale para o modo panorama para enxergar de forma matricial todas as salas e sua ocupação ao longo do dia de hoje.</li>
                          <li><strong>Reservas Diretas:</strong> Cadastre agendamentos exclusivos (reuniões, palestras, contraturnos, etc.) preenchendo o professor responsável e a atividade, eliminando bloqueios ou duplicidades no mesmo período.</li>
                        </ul>
                      </div>
                    </div>
                  )}
