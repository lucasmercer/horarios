...
export default function Certificados() {
  ...
  const [schoolName, setSchoolName] = useState(() => localStorage.getItem('cecm_school_name') || 'Colégio Estadual Cívico-Militar Gregório Szeremeta');
  const [additionalText, setAdditionalText] = useState(() => getDefaultDescription(schoolName));
  ...
  useEffect(() => {
    ...
    if (savedLogo) {
      if (savedLogo.startsWith('data:image/png')) {
        try {
          const base64 = savedLogo.split(',')[1];
        ...
        // Warning: this might fail for cross-origin URLs not supporting CORS
        fetch(savedLogo)
          .then(res => res.arrayBuffer())
          .then(buf => setLogoBytes(new Uint8Array(buf)))
          .catch(err => console.warn("Could not load logo due to CORS or network", err));
      }
    }
  }, []);
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4">
        ...
        >
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500 text-sm font-medium text-center">{schoolName}</p>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[8px] lg:text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
              {schoolName.replace('Colégio Estadual Cívico-Militar', 'CECM').replace('Colégio Estadual', 'CE')}
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col lg:flex-row lg:overflow-hidden overflow-y-auto">
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-1.5 max-h-44 overflow-y-auto pr-1">
                    {getMensagensPresets(schoolName).map((p) => {
        ...
        <section className="flex-1 p-6 lg:p-10 flex flex-col items-center justify-center lg:overflow-hidden bg-[#F1F5F9]">
          ...
          <div className="w-full max-w-4xl h-8 flex items-center justify-between text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-4 shrink-0 px-1">
            ...
            <span className="hidden sm:block">{schoolName.replace('Colégio Estadual Cívico-Militar', 'CECM').replace('Colégio Estadual', 'CE')} - Certificados de Excelência</span>
          </div>
        </section>
      </main>
    </div>
