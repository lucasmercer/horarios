...
export default function SalasInfrastructure() {
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => {
      ...
      // Get all parent CSS to ensure classes like border-[3px] render correctly
      ...
      const htmlContent = `
          ...
          <style>
            ...
            * {
            ...
            @media print {
              ...
              @page { margin: 10mm; size: landscape A4; }
              
              /* Ensure the internal container does not wrap and break into 4 pages */
              .map-wrapper {
                width: 1400px !important;
                zoom: 0.75; /* Scales the 1400px down to fit perfectly on A4 landscape */
                page-break-inside: avoid;
              }

              /* Strip only the overflow for the main scroll block to prevent scrollbars on paper, keep aesthetics */
              #print-school-map-container > div {
        ...
        <body>
          <div class="map-wrapper text-center">
            <h2 style="margin-bottom: 20px; font-weight: 900; color: #50622a; font-size: 24px; text-transform: uppercase;">PLANTA DE AMBIENTES FÍSICOS - MAPA ESCOLAR</h2>
        ...
        <>
            <div className="space-y-6">
              ...
              {mapViewStyle === "gregorio" ? (
                ...
                <div className="p-6 md:p-8 rounded-3xl border-[3px] border-[#50622a] shadow-md relative flex flex-col gap-6 transition-all duration-300 overflow-x-auto bg-[#faf9f5]"
                  style={{
                ...
                /* ORIGINAL DYNAMIC LAYOUT BLOCKS BLUEPRINT RENDERER */
                <div
                  className="p-6 md:p-8 rounded-3xl border-2 border-slate-300 min-h-[500px] shadow-inner relative flex flex-col gap-8 transition-colors duration-300 overflow-x-auto"
                  style={{
                    backgroundColor: "#0c1a30", // Grid Blueprint blueprint theme
                    backgroundImage:
                      "radial-gradient(#1e3a6a 1px, transparent 1px)",
                    backgroundSize: "16px 16px",
                  }}
                >
                  {/* Blueprint Header HUD details */}
                  <div className="flex items-center justify-between border-b border-[#224483] pb-3 text-white">
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full bg-emerald-400 animate-ping font-extrabold" />
                      <div>
                        <span className="text-[9px] font-black text-cyan-400 uppercase tracking-widest block font-mono">
                          MAP LEVEL ACTIVE HUD
                        </span>
                        <h3 className="text-xs font-black uppercase text-slate-200 tracking-wider">
                          PLANTA DE IMPACTO DA INFRAESTRUTURA FISICA •{" "}
                          {DAYS.find((d) => d.id === selectedDayId)?.label}
                        </h3>
                      </div>
                    </div>

                    <div className="flex items-center gap-4 text-[9px] font-mono text-cyan-300 select-none">
                    ...
                    {Object.keys(groupedBlocks).length === 0 ? (
                      ...
                      Object.keys(groupedBlocks).map((blockName) => {
                        ...
                        return (
                          <div
                            key={blockName}
                            className="bg-slate-900/40 border border-[#224483] rounded-2xl p-5 relative overflow-hidden transition"
                          >
                            {/* Block Outer Labeled Border */}
                            <div className="absolute top-2 right-4 text-[9px] font-black text-cyan-400 uppercase tracking-widest font-mono">
                              ZONE: {blockName}
                            </div>

                            <div className="mb-4">
                              <span className="text-[11px] font-black text-white uppercase tracking-wider block font-sans bg-slate-900 border border-cyan-800/60 w-max px-3 py-1 rounded-md shadow-md">
                                🏢 {blockName}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-4">
                              {sortedFloorKeys.map((floorName) => {
                                ...
                                return (
                                  <div
                                    key={floorName}
                                    className="border-t border-dashed border-[#1f3d73]/60 pt-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-4"
                                  >
                                    {/* Floor Level Name Plate */}
                                    <div className="w-24 text-left font-sans shrink-0 border-r border-[#1f3d73] pr-2">
                                      <span className="text-[10px] font-black text-cyan-300 uppercase block font-mono">
                                        LEVEL FLAG
                                      </span>
                                      <span className="text-xs font-black text-slate-350 uppercase truncate block">
                                        {floorName}
                                      </span>
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-wrap gap-3">
                                      {floorRooms.map((room) => {
                                        ...
                                        return (
                                          <div
                                            key={room.id}
                                            className={`relative rounded-xl p-3 flex flex-col justify-between font-sans border-2 min-w-[140px] max-w-[200px] flex-1 min-h-[90px] transition-all group ${
                                              occupancyLabel
                                                ? "bg-amber-950/20 border-amber-500/80 text-amber-250 shadow-[0_0_12px_rgba(245,158,11,0.15)]"
                                                : "bg-[#10b981]/10 border-emerald-500/80 text-emerald-250 shadow-[0_0_12px_rgba(16,185,129,0.15)]"
                                            }`}
                                          >
                                            {/* Quick Edit Positioning form inline */}
                                            {isEditingLayout ? (
                                              <div className="space-y-1.5 text-xs text-left bg-slate-950 p-2 rounded-lg border border-cyan-800 z-20 absolute inset-0 text-white">
                                                <div className="flex justify-between items-center bg-slate-900 px-1 py-0.5 rounded text-[8px] font-black uppercase text-cyan-300">
                                                ...
                                                <div className="space-y-1">
                                                  <input
                                                    ...
                                                    placeholder="Bloco"
                                                    className="w-full text-[9px] font-bold px-1.5 py-1 bg-slate-900 border border-slate-700 rounded text-slate-200 outline-none"
                                                  ...
                                                  <button
                                                    ...
                                                    className="w-full text-[8.5px] font-black bg-cyan-600 text-white rounded py-1 uppercase"
                                                  >
                                                    Salvar Alinhamento
                                                  </button>
                                                </div>
                                            ...
                                            <div className="space-y-1">
                                              <div className="flex justify-between items-start gap-1">
                                                <span className="text-[10.5px] font-black uppercase tracking-tight truncate max-w-[85%] text-slate-100">
                                                  {room.name}
                                                </span>
                                              ...
                                              <p className="text-[9px] font-bold text-slate-300 line-clamp-2 leading-snug mt-1 font-mono">
                                                {occupancyLabel
                                                  ? occupancyLabel
                                            ...
                                            <div className="mt-2 pt-1 border-t border-[#1f3d73]/30 flex items-center justify-between text-[7px] font-black uppercase tracking-wider">
                                              <span
                                                className={
                                                  occupancyLabel
                                                    ? "text-amber-400"
                  ...
                  <div className="text-[8px] font-mono text-cyan-500 border-t border-[#224483] pt-2 flex justify-between items-center">
                    <span>
                      SYSTEM_COORDS // REAL-TIME PLANTA DISCIPLINAR SYSTEM
                    </span>
                    <span>TOTAL_ESTUDIOS_MONITORADOS: {rooms.length}</span>
                  </div>
                </div>
              )}
              </div>
              ...
              {inspectRoomId && (
                (() => {
                  ...
                  return (
                    <div className="bg-slate-900 text-white rounded-2xl border border-slate-800 p-5 shadow-lg text-left animate-in slide-in-from-bottom duration-350">
                      <div className="flex justify-between items-start border-b border-slate-800 pb-3 mb-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-emerald-550/10 border border-emerald-450 flex items-center justify-center text-emerald-400">
                            <Building2 className="w-5 h-5" />
                          </div>
                          <div>
                            <span className="text-[8px] font-black tracking-wider text-emerald-400 block font-mono uppercase">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 text-[8.5px] font-black bg-slate-805 text-slate-400 hover:text-white rounded-md uppercase tracking-wider transition cursor-pointer"
                        >
                          Fechar Painel
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-[9px] font-black text-slate-400 uppercase block tracking-wider mb-2">Nome da Instalação</label>
                            <input type="text" value={editRoomName} onChange={e => setEditRoomName(e.target.value)} className="w-full bg-slate-950 border border-slate-700/50 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500 transition-colors" placeholder="Ex: Sala 1, Lab 2" />
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-[9px] font-black text-slate-400 uppercase block tracking-wider mb-2">Turma Base (Manhã)</label>
                            <select value={editAssignedTurmas.manha || ""} onChange={e => setEditAssignedTurmas(prev => ({...prev, manha: e.target.value}))} className="w-full bg-slate-950 border border-slate-700/50 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500 transition-colors">
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-[9px] font-black text-slate-400 uppercase block tracking-wider mb-2">Turma Base (Tarde)</label>
                            <select value={editAssignedTurmas.tarde || ""} onChange={e => setEditAssignedTurmas(prev => ({...prev, tarde: e.target.value}))} className="w-full bg-slate-950 border border-slate-700/50 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500 transition-colors">
                              <option value="">-- Livre / S.D. --</option>
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-[9px] font-black text-slate-400 uppercase block tracking-wider mb-2">Turma Base (Noite)</label>
                            <select value={editAssignedTurmas.noite || ""} onChange={e => setEditAssignedTurmas(prev => ({...prev, noite: e.target.value}))} className="w-full bg-slate-950 border border-slate-700/50 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500 transition-colors">
                              <option value="">-- Livre / S.D. --</option>
                      ...
                      <div className="mt-6 pt-4 border-t border-slate-800 flex justify-end gap-3.5">
                        <button
                          type="button"
                          onClick={() => setInspectRoomId(null)}
                          className="bg-transparent hover:bg-slate-800 text-slate-400 font-bold text-[10px] uppercase py-2.5 px-5 rounded-xl transition-all cursor-pointer"
                        >
                          Cancelar
                        </button>
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] uppercase py-2.5 px-6 rounded-xl transition-all cursor-pointer shadow-md"
                        >
